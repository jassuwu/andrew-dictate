...
- **cleaner (deterministic, always on — ADR 0004, 0019):** eleven staged transforms — dictionary substitutions, spoken punctuation, filler removal, self-correction *detection*, repetition collapse, email/url/number parsing, capitalization, punctuation finishing, whitespace normalization. the self-correction stage flags and routes rather than rewriting: it acts on nothing but a whole utterance of "scratch that", because every marker it knows is also an ordinary english word (ADR 0019 amendment, 2026-08-22). microseconds, no model, pure functions. not user-disableable: it is where the dictionary and spoken punctuation live, and without it you get raw asr.
